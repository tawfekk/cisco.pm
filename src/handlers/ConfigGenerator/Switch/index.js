...
export function Topcustomconfig(index) {
  try {
    if (
      JSON.parse(localStorage.switch_data)[index]["misc"][2]["customconfig"]
    ) {
      let workingvar =
        "\n\n" +
        JSON.parse(localStorage.switch_data)[index]["misc"][2]["customconfig"];
      let workingdata = JSON.parse(localStorage.switch_final);
      workingdata[index]["topcustomconfig"] = workingvar;
      localStorage.switch_final = JSON.stringify(workingdata);
      return workingvar;
    }else{
      let workingdata = JSON.parse(localStorage.switch_final);
...
export function Initial(index) {
  try {
    var today = new Date();
    var workingvar = "\n\nend\n";
    var form = JSON.parse(localStorage.switch_data)[index]["initial"][0];
    if (form.clock == true) {
      workingvar +=
        "clock set " +
        today.getHours() +
...
export function Interfaces(index) {
  try {
    var workingvar = "";
    for (const e of JSON.parse(localStorage.switch_data)[index]["interfaces"]) {
      if (e.port) {
        workingvar += "\n\ninterface " + e.port;
        if (!isIPv6(e.ip)) {
    ...
    for (const e of JSON.parse(localStorage.switch_data)[index][
      "linterfaces"
    ]) {
...
export function DHCP(index) {
  try {
    var workingvar = "";
    for (const e of JSON.parse(localStorage.switch_data)[index]["dhcp"]) {
    ...
    for (const e of JSON.parse(localStorage.switch_data)[index]["dhcpexclusion"]) {
      if (e.from && e.to) {
        workingvar += "\nip dhcp excluded-address " + e.from + " " + e.to;
      }
    }
    for (const e of JSON.parse(localStorage.switch_data)[index]["dhcphelper"]) {
...
export function FHRP(index) {
  try {
    var workingvar = "";
    for (const e of JSON.parse(localStorage.switch_data)[index]["hsrp"]){
    ...
    for (const e of JSON.parse(localStorage.switch_data)[index]["vrrp"]){
      if (e.group && e.ip && e.priority) {
        workingvar +=
          "\n\ninterface " +
          e.port +
...
export function OSPF(index) {
  try {
    var workingvar = "";
    for (const e of JSON.parse(localStorage.switch_data)[index]["ospf"]) {
      if (e.enabled && !e.ipv6 ) {
        workingvar += "\n\nswitch ospf " + e.processid;
        if (e.defaultroute) {
    ...
    for (const e of JSON.parse(localStorage.switch_data)[index]["v3ospf"]) {
      if (e.enabled || e.enabledv6) {
      workingvar += "\n\nipv6 unicast-routing"
      workingvar += "\nswitch ospfv3 " + e.processid
      if(e.key){workingvar += "\narea "+ e.area + " authentication ipsec spi 2048 sha1 "+ SHA1(e.key)}
      if(e.override){workingvar += "\nswitch-id " + e.override}else{workingvar += "\nno switch-id"}
      if (e.enabled){
...
export function EIGRP(index) {
  try {
    var workingvar = "";
    for (const e of JSON.parse(localStorage.switch_data)[index]["eigrp"]) {
      if (e.as && !e.ipv6) {
        workingvar += "\n\nswitch eigrp " + e.as;
        if(e.autosummary){
          workingvar += "\nauto-summary";
  ...
  for (const e of JSON.parse(localStorage.switch_data)[index]["namedeigrp"]) {
    if (e.as) {
...
export function NAT(index) {
  try {
    var workingvar = "";
    for (const e of JSON.parse(localStorage.switch_data)[index]["dynamicnat"]) {
      if (
        e.interalip &&
        e.internalsubnet &&
        e.externalendip &&
    ...
    for (const e of JSON.parse(localStorage.switch_data)[index]["staticnat"]) {
...
function Customconfig(index) {
  try {
    if (
      JSON.parse(localStorage.switch_data)[index]["misc"][0]["customconfig"]
    ) {
      let workingvar =
        "\n" +
        JSON.parse(localStorage.switch_data)[index]["misc"][0]["customconfig"];
...
export function BGP(index) {
  try {
    ...
    var workingvar = "";
    for (const e of JSON.parse(localStorage.switch_data)[index]["bgp"]) {
      if(e.as && e.ipv6){
      workingvar += "\n\nswitch bgp "+ e.as
      if(e.switchid){
        workingvar += "\nbgp switch-id " + e.switchid;
      }else{
        workingvar += "\nno bgp switch-id";
      }
...
export function Security(index) {
  ...
  let workingvar = "";
  /*
  let data = JSON.parse(localStorage.switch_data)[index]["basicsecurity"]; 
  if(data.autosecure){
    workingvar += "\n\nauto secure";
  }else{workingvar += "\nno auto secure";}
  ...
  */
  ...
  if(JSON.parse(localStorage.switch_data)[index]["localaaa"][0]["users"]){
    let data = JSON.parse(localStorage.switch_data)[index]["localaaa"][0]
    if(JSON.parse(localStorage.switch_data)[index]["localaaa"][0]["users"].length > 0){
    workingvar += "\n\naaa new-model"
    for (const e of JSON.parse(localStorage.switch_data)[index]["localaaa"][0]["users"]){
      if(!e.privilege){
  ...
  for (const e of JSON.parse(localStorage.switch_data)[index]["advancedaaa"]){
   if (e.protocol == "radius"){
    if (JSON.parse(localStorage.switch_data)[index]["basicsettings"][0]["model" === "1931"]){
