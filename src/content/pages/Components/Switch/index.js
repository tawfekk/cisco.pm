...
function Initial() {
  try {
    var today = new Date();
    var workingvar = "\n";
    var workingarr = JSON.parse(localStorage.switch_data)[
    ...
    if (workingarr.motd != "") {
      ...
      workingdata[sessionStorage.switch_tabid]["initial"] = workingvar;
...
function Interfaces() {
  try {
    var workingvar = "";
    for (const element of JSON.parse(localStorage.switch_data)[
      sessionStorage.switch_tabid
    ]["interfaces"]) {
      workingvar +=
        "\ninterface range " +
    ...
    let workingdata = JSON.parse(localStorage.switch_final);
    workingdata[sessionStorage.switch_tabid]["interfaces"] = workingvar;
    localStorage.switch_final = JSON.stringify(workingdata);
    return workingvar;
  } catch (error) {}
}
...
function DHCP() {
  try {
    ...
    let workingdata = JSON.parse(localStorage.switch_final);
    workingdata[sessionStorage.switch_tabid]["dhcp"] = workingvar;
    localStorage.switch_final = JSON.stringify(workingdata);
    return workingvar;
  } catch (error) {}
}
...
function Switch() {
  function tablabel(maxTabIndex) {
    try {
      if (
        JSON.parse(localStorage.switch_data)[maxTabIndex]["initial"][0][
          "hostname"
        ]
      ) {
        return JSON.parse(localStorage.switch_data)[maxTabIndex]["initial"][0][
          "hostname"
        ];
      } else {
        return "Ny switch";
  ...
  const handleTabChange = (event, newtabid) => {
    if (newtabid === "tabProperties") {
      ...
      let object = {
        interfaces: [{ porte: [] }],
        dhcp: [{ ip: "" }],
        initial: [{ hostname: "S" + workingtabindex }],
      };
      data.push(object);
      setformFields(data);
      syncupdate(data)
      ...
      data2.push({ initial: "" });
      localStorage.switch_final = JSON.stringify(data2);
      handleAddTab();
    } else {
  ...
  function onreloadtab() {
    let tabdata = [...tabs];
    if (JSON.parse(localStorage.switch_data).length != 1) {
      for (var i = 1; i < JSON.parse(localStorage.switch_data).length; i++) {
  ...
  window.onload = function () {
    //setformFields[tabid](JSON.parse(localStorage.switch_data));
    sync();
  };
  ...
  async function syncupdate(data) {
    if (sessionStorage.sessionid) {
      try {
        await setDoc(doc(db, sessionStorage.sessionid, "switch"), {
          data: [...data],
        });
      } catch (e) {
        console.log(e);
  ...
  async function sync() {
    try {
      if (sessionStorage.sessionid) {
        const docRef = doc(db, sessionStorage.sessionid, "switch");
        const docSnap = await getDoc(docRef);
        setformFields(docSnap.data()["data"]);
        localStorage.switch_data = JSON.stringify(docSnap.data()["data"]);
      }
      //else {
  ...
  function modalclipboardcontent(id) {
    return (
      "conf terminal" +
      JSON.parse(localStorage.switch_final)[sessionStorage.switch_tabid][id] +
      "\nend"
    );
  }
    ...
    return (
      <div>
        ...
        <TabPanel value={value} index={0}>
          <Card>
            <CardHeader title="Basale switch indstillinger" />
            <Divider />
            <CardContent>
              <Box
                component="form"
              ...
              >
                <Box sx={style}>
                  ...
                  <Button
                    onClick={() => {
                      navigator.clipboard.writeText(
                        localStorage.switch_DHCP_final
                      );
                    }}
                    variant="contained"
                    sx={{ right: "20%", left: "20%", margin: 2 }}
