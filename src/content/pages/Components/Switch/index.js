...
sessionStorage.switch_tabid = 0;


/*if (
  JSON.parse(localStorage.switch_final).length !=
  JSON.parse(localStorage.switch_data).length
) {
  var times = JSON.parse(localStorage.switch_data).length;
  let data = JSON.parse(localStorage.switch_final);
  ...
  localStorage.switch_final = JSON.stringify(data);
}
*/
...
function Switch() {
  function tablabel(maxTabIndex) {
    try {
      let data = JSON.parse(localStorage.switch_data);
      let workingdata = data[maxTabIndex]["initial"][0]["hostname"];
      if (workingdata) {
        return workingdata;
      } else {
        let switchid = maxTabIndex + 1;
        return "R" + switchid;
        data[2]["initial"][0]["hostname"] = "R" + switchid;
  ...
  const handleTabChange = (event, newtabid) => {
    if (newtabid === "tabProperties") {
      ...
      sessionStorage.switch_tabid = newtabid;
      settabid(newtabid);
      run2();
    }
  };
  ...
  function onreloadtab() {
    let tabdata = [...tabs];
    if (
      JSON.parse(localStorage.switch_data).length != 1 &&
      JSON.parse(localStorage.switch_data).length != tabdata.length + 1
    ) {
      while (
        JSON.parse(localStorage.switch_data).length !=
        tabdata.length + 1
  ...
  async function run() {
    ...
    setValue(value);
    setformFields(JSON.parse(localStorage.switch_data));
  }
  ...
  const handleFormChange = (event, index, value) => {
    ...
    if (event.target.type == "checkbox") {
      data[sessionStorage.switch_tabid][event.target.id][index][
        event.target.name
      ...
      var id = event.target.id;}else{
        ...
        id = parsed[0],
        name = parsed[1];
        data[sessionStorage.switch_tabid][id][index][name] = event.target.value;
      }
    } else {
      ...
      var parsed = splitcandidate.split("."),
        id = parsed[0],
        name = parsed[1];
      data[sessionStorage.switch_tabid][id][index][name] = valuecandidate;
    }
    setformFields(data);
    localStorage.switch_data = JSON.stringify(data);
    if (sessionStorage.sessionid) {
      syncupchange(
        sessionStorage.switch_tabid,
        id,
        data[sessionStorage.switch_tabid][id],
        "switch"
      );
      //syncup(data, 'switch')
    }
  ...
  const handleNestedFormChange = (nest, nestindex, event, index, value) => {
    ...
    //if (data[0][event.target.id][index] == undefined) {data[0][event.target.id] = {}}
    if (event.target.type == "checkbox") {
      data[sessionStorage.switch_tabid][nest][nestindex][event.target.id][
        index
      ...
      id = parsed[0],
      name = parsed[1];
      data[sessionStorage.switch_tabid][nest][nestindex][id][index][name] = event.target.value;
    }
      ...
      var parsed = splitcandidate.split("."),
        id = parsed[0],
        name = parsed[1];
      data[sessionStorage.switch_tabid][nest][nestindex][id][index][name] =
        valuecandidate;
    }
    setformFields(data);
    localStorage.switch_data = JSON.stringify(data);
    if (sessionStorage.sessionid) {
      syncupchange(
        sessionStorage.switch_tabid,
        id,
        data[sessionStorage.switch_tabid][nest][nestindex][id],
        "switch",
        nest,
        nestindex
      );
  ...
  const addFields = (id) => {
    ...
    let object = {
      ...
      area: 0,
      index: data[sessionStorage.switch_tabid][id].length,
    };
    data[sessionStorage.switch_tabid][id].push(object);
    setformFields(data);
  };
  ...
  function addNestedFields(nest, index, id) {
    ...
    data[sessionStorage.switch_tabid][nest][index][id].push(object);
  ...
  const removeNestedFields = (nest, nestindex, id, index) => {
    let data = [...formFields];
    data[sessionStorage.switch_tabid][nest][nestindex][id].splice(index, 1);
    setformFields(data);
    localStorage.switch_data = JSON.stringify(data);
    syncupchange(
      sessionStorage.switch_tabid,
  ...
  const removeFields = (id, index) => {
    let data = [...formFields];
    data[sessionStorage.switch_tabid][id].splice(index, 1);
    setformFields(data);
    localStorage.switch_data = JSON.stringify(data);
    syncupchange(sessionStorage.switch_tabid, id, 0, "switch", 0, 0, index);
  };
...
window.onload = (event) => {
  if (sessionStorage.sessionid){ setTimeout(() => {setformFields(JSON.parse(localStorage.switch_data))}, 400)}
};
  ...
  function porte(sort) {
    try {
      let workingvar = SwitchInterfaces(
    ...
    return (
      <Box sx={style}>
        ...
        <TextField
          ...
          style={{ width: "100%" }}
          value={
            id != "initial" ? "configure terminal" + func(sessionStorage.switch_tabid) + "\nend" : "configure terminal" + func(sessionStorage.switch_tabid) + RSA(sessionStorage.switch_tabid) + "\nend" 
          }
        ></TextField>
        <Button
          onClick={() => {
            navigator.clipboard.writeText(
              "conf terminal" +
                JSON.parse(localStorage.switch_final)[
                  sessionStorage.switch_tabid
                ][id] +
                "\nend"
            );
    ...
    return (
      <div>
        ...
        <Tabs
          ...
          value={value}
          //style={{ height: '100%', width: 100, display: "flex", m: 2 }} 
        ...
        >
          <Tab label="Generel" />
          <Tab label={formFields[tabid]["initial"][0]["mls"] ? "L2 interfaces": "Interfaces"} />
          <Tab sx={{display: formFields[tabid]["initial"][0]["mls"] ? "": "none"}} label="L3 interfaces" />
          <Tab sx={{display: formFields[tabid]["initial"][0]["mls"] ? "": "none"}} label="DHCP" />
          <Tab sx={{display: formFields[tabid]["initial"][0]["mls"] ? "": "none"}} label="FHRP" />
          <Tab sx={{display: formFields[tabid]["initial"][0]["mls"] ? "": "none"}} label="Static route" />
          <Tab sx={{display: formFields[tabid]["initial"][0]["mls"] ? "": "none"}} label="OSPF" />
          <Tab sx={{display: formFields[tabid]["initial"][0]["mls"] ? "": "none"}} label="EIGRP" />
          <Tab sx={{display: formFields[tabid]["initial"][0]["mls"] ? "": "none"}} label="BGP" />
          <Tab sx={{display: formFields[tabid]["initial"][0]["mls"] ? "": "none"}} label="VPN" />
          <Tab sx={{display: formFields[tabid]["initial"][0]["mls"] ? "": "none"}} label="ACL & routemap" />
          <Tab sx={{display: formFields[tabid]["initial"][0]["mls"] ? "": "none"}} label="NAT" />
          <Tab label="Security" />
          <Tab label="Misc" />
        </Tabs>
          ...
          <Card >
            <CardContent >
              <Box
                ...
                autoComplete="off"
              >
                {formFields[sessionStorage.switch_tabid]["initial"].map(
                  (form, index) => {
                    return (
                      <div key={0}>
                         <TextField
                        ...
                        <FormControl sx={{ mr: 1, ml: 1.2, mt: 1, width: 220 }}>
                          ...
                          >
                            <MenuItem key={"2960"} value={"2960"}>
                              2960
                            </MenuItem>
                            <MenuItem key={"35XX"} value={"35XX"}>
                              35XX
                            </MenuItem>
                          </Select>
                        </FormControl>
                        ...
                        <Grid
                          ...
                          justifyContent="center"
                        >
                          <Tooltip arrow title="Defines if this switch should have multilayer capabilities">
                          <FormControlLabel
                            color="info"
                            sx={{ m: 1.5 }}
                            control={
        ...
        <TabPanel value={value} index={formFields[tabid]["initial"][0]["mls"] ? 3: ""}>
        ...
        <TabPanel value={value} index={formFields[tabid]["initial"][0]["mls"] ? 4: ""}>
        {formFields[tabid]["hsrp"].map((form, index) => {
                ...
                <CardContent>
                  <div key={index}>
                    ...
                    >
                      ...
                      <Tooltip arrow title="Den fælles IP-adresse, som HSRP-gruppen bruger som switchens identitet. Denne IP-adresse flyttes mellem switchne afhængigt af, hvilken switch der fungerer som master.">
                      <TextField
                        required
                        name="ip"
                        error={!form.ip}
                      ...
                      <Tooltip arrow title="Det numeriske ID, der identificerer en specifik HSRP-gruppe. Alle switche i samme VRRP-gruppe deler den virtuelle IP-adresse og arbejder sammen for at opretholde Networksredundans.">
                      <TextField
                        required
                        name="group"
                        id="hsrp"
                      ...
                      <Tooltip arrow title="Det tal, der bestemmer, hvilken switch der bliver master. Højere tal betyder højere prioritet.">
                      <TextField
                      required
                        name="priority"
                        id="hsrp"
                    ...
                    <Tooltip arrow title="preempt betyder at den switch med højeste prioritet kan overtage som master, hvis den bliver tilgængelig igen efter en fejl">
                    <FormControlLabel
                        labelPlacement="bottom"
                        control={
                          <Checkbox
                ...
                <CardContent>
                  <div key={index}>
                    ...
                    >
                      ...
                      <Tooltip arrow title="Den fælles IP-adresse, som VRRP-gruppen bruger som switchens identitet. Denne IP-adresse flyttes mellem switchne afhængigt af, hvilken switch der fungerer som master.">
                      <TextField
                        required
                        name="ip"
                        error={!form.ip}
                      ...
                      <Tooltip arrow title="Det numeriske ID, der identificerer en specifik VRRP-gruppe. Alle switche i samme VRRP-gruppe deler den virtuelle IP-adresse og arbejder sammen for at opretholde Networksredundans.">
                      <TextField
                        required
                        name="group"
                        id="vrrp"
          ...
          <Tooltip arrow title="HSRP (Hot Standby Switch Protocol): Aktiv/standby-switch, statisk switch ID-tildeling, primært til Cisco-udstyr.">
          <Button
            variant="contained"
            sx={{ margin: 1 }}
            size="medium"
          ...
          <Tooltip arrow title="VRRP (Virtual Switch Redundancy Protocol): Aktiv/standby-switch, dynamisk switch ID-tildeling, åben standard, understøttet af flere leverandører.">
          <Button
            variant="contained"
            sx={{ margin: 0.2 }}
            size="medium"
        ...
        <TabPanel value={value} index={formFields[tabid]["initial"][0]["mls"] ? 5: ""}>
        ...
        <TabPanel value={value} index={formFields[tabid]["initial"][0]["mls"] ? 6: ""}>
                ...
                <CardContent>
                  <div key={index}>
                    ...
                    >
                      ...
                      <Tooltip arrow title="Unik identifikator for OSPF-switchen. Brug en unik IPv4-adresse som switch-id, f.eks. en loopback-adresse.">
                      <TextField
                        name="override"
                        id="ospf"
                        placeholder="1.1.1.1"
                        label="Switch ID override"
                        onChange={(event) => handleFormChange(event, index)}
                        value={form.override || ''}
                      />
                      </Tooltip>
                      ...
                      <Tooltip arrow title="Prioritet tildelt OSPF-switchen ved valg af Designated Switch (DR) og Backup Designated Switch (BDR). Højere prioritet øger chancen for at blive valgt som DR eller BDR.">
                      <TextField
                        name="priority"
                        id="ospf"
                        type="number"
                      ...
                      <Tooltip arrow placement="top" enterDelay={1000} title="Angivelse af de interfaces, hvor OSPF ikke skal sende 'Hello' / deltage i DR/BDR-valg. Brug dette til Network, hvor der ikke er forbundet en OSPF switch, men som stadig skal annonceres">
                      <FormControl sx={{ mr: 1, ml: 1.2, mt: 1, width: 218 }}>
                      ...
                      <Tooltip arrow placement="top" enterDelay={1000} title="Enable on interfaces that are directly connected to another switch without passing thorugh any networks.">
                      <FormControl sx={{ mr: 1, ml: 1.2, mt: 1, width: 218 }}>
                      ...
                      <Tooltip arrow title="Does switch lead to the internet, or should it be the gateway of last resort? If yes, enable this">
                      <FormControlLabel
                        labelPlacement="bottom"
                        sx={{ m: 1.5 }}
                        control={
                ...
                <CardContent>
                  <div key={index}>
                    ...
                    >
                      ...
                      <Tooltip arrow placement="top" enterDelay={1000} title="Angivelse af de interfaces, hvor OSPF ikke skal sende 'Hello' / deltage i DR/BDR-valg. Brug dette til netværk, hvor der ikke er forbundet en OSPF switch, men som stadig skal annonceres">
                      <FormControl sx={{ mr: 1, ml: 1.2, mt: 1, width: 218 }}>
                        <InputLabel>Passive interfaces</InputLabel>
                        <Select
                          name="v3ospf.passive"
                ...
                <CardContent>
                  <div key={index}>
                    ...
                    >
                      ...
                      <Tooltip arrow placement="top" enterDelay={1000} leaveDelay={0}  title={form.ipv6 ? "A 32-bit value, to identify this switch - in IPv6, this is required" : "A 32-bit value, to identify this switch"}>
                      <TextField
                        name="switchid"
                        error= {!form.switchid && form.ipv6}
                        required={form.ipv6}
                        id="eigrp"
                        placeholder="1.1.1.1"
                        label="Switch ID"
                      ...
                      <Tooltip arrow  title="Define a secret key - this needs to be the same across all EIGRP switchs">
                      <TextField
                        name="key"
                        id="eigrp"
                      ...
                      <Tooltip arrow title="Aggregate contiguous subnets into a single summarized route, simplifying the routing table and reducing its size when advertising networks to other switchs">
                      <FormControlLabel
                        labelPlacement="bottom"
                        sx={{ m: 1.5 }}
                        control={
                ...
                <CardContent>
                  <div key={index}>
                    ...
                    >
                      ...
                      <Tooltip arrow  title="Define a secret key - this needs to be the same across all EIGRP switchs - please be aware that authkeys are only applied on explitly enabled interfaces">
                      <TextField
                        name="key"
                        id="namedeigrp"
                        label="Authentication key"
                ...
                <CardContent>
                  <div key={index}>
                    ...
                    >
                      ...
                      <Tooltip arrow placement="top" enterDelay={1000} leaveDelay={0}  title="Switchens ID er en 32-bit værdi, der bruges til at identificere switchen unikt inden for et AS">
                      <TextField
                        name="switchid"
                        id="bgp"
                        placeholder="1.1.1.1"
                      ...
                      <Tooltip arrow title="Defines a MD5 hased key to use - this needs to be the same across all BGP switchs">
                      <TextField
                        name="key"
                        id="bgp"
                        disabled={!form.ipv6}
                      ...
                      <Tooltip arrow title="Activate if this switch should be the gateway of last resort, e.g. if its connected to the internet">
                      <FormControlLabel
                        labelPlacement="bottom"
                        sx={{ m: 1.5 }}
                        control={
                ...
                <CardContent>
                  <div key={index}>
                    <Box // sx={{ width: '100%' }}
                      ...
                      autoComplete="off"
                    >
                  <Tooltip arrow title="Automates the process of securing the switch by applying recommended settings.">
                    <FormControlLabel
                      labelPlacement="bottom"
                      sx={{ m: 1.5 }}
                      control={
            ...
            >
                ...
                <Card>
                  ...
                  <CardContent sx={{ width: 1085, justifyContent: "center" }}>
                    ...
                    >
                      <TextField
                        label="Notes about this switch (will not included in the final configuration)"
                        multiline
                        maxRows={Infinity}
                        minRows={5}
                        style={{ width: "97%" }}
